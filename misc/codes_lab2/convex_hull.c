...
struct data *merge(struct data *left, struct data *right) {
  ...
  //  printf("highleft %f %f\n",highleft->p.x,highleft->p.y);
   ...
   // printf("lowright1 %f %f\n",tempr->p.x,tempr->p.y);
    while(slope(templ->p,tempr->p)==INF&&tempr!=highright)	//vertical line 
    {
    	tempr=tempr->next;
    }
  //  printf("lowleft2 %f %f\n",templ->p.x,templ->p.y);
    //printf("lowright2 %f %f\n",tempr->p.x,tempr->p.y);
    while(slope(templ->p,tempr->p)==slope(templ->p,tempr->next->p)&&tempr!=highright&&tempr->next->p.y>0)	//checking for +ve quadrant
    {
    	tempr=tempr->next;
    }
   while(slope(templ->p,tempr->p)==slope(templ->next->p,tempr->p)&&templ->next->p.x<templ->p.x&&templ->next->p.y==templ->p.y)	//checking for -ve quadrant also
    {
    	templ=templ->next;
    }
    lowleft = templ;
...
//includes dupliacte points as well
int main()
{
    int j,n,i,t,di;
    double dis;
