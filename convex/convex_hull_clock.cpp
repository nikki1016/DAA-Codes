...
/*	cout << "Convex_left is " << endl;
	...
	for (int i = 0; i < convex_left.size(); i++){
		if(convex_left[i].first == xlowl && convex_left[i].second == ylowl){
			index = i;
			break;
		}
	}
...
//	cout << "convex_left.size" << convex_left.size() << endl;
	while(convex_left[i].first != xhighl || convex_left[i].second != yhighl){
...
//	cout << i << endl;
	...
	*/
	for (int i = 0;i < convex_right.size(); i++){
		...
		if(convex_right[i].first == xhighr && convex_right[i].second == yhighr ){
			index = i;
			break;
		}
	}

	while (convex_right[index].first != xlowr ||  convex_right[index].second != ylowr){
